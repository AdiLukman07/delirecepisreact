...
import RecipeSearchResultCard from "../components/RecipeSearchResultCard";
...
export default function SearchDetails() {
  ...
  const performSearch = async (query: string) => {
    ...
    // untuk menghilangkan recipe yang di cari
    if (!query) {
        setSearchResult([]);
        return;
    }
        ...
        <div className="flex flex-col gap-[18px] mt-[18px]">
            ...
            {searchResult.length > 0 ? (
                searchResult.map((recipe) => (
                    <RecipeSearchResultCard key={recipe.id} recipe={recipe}></RecipeSearchResultCard>
